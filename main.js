getJIRAFeed = (callback, errorCallback) => {
...
buildJQL = (callback) => {
  ...
  var fullCallbackUrl = callbackBase + 'project=' + project + '&status=' + status + '&inStatusFor=' + inStatusFor;
  callback(fullCallbackUrl);
}
createHTMLElementResult = (response) => {
  var results = '';
  for (var i = 0; i < response.issues.length; i ++) {
    results += "<div>" + "<p style='margin-right: 5px; width: 80px;     margin-bottom: 0;'>" +  response.issues[i].key + "</p>" + "<p style='margin-right: 5px; width: 80px; margin-bottom: 0;'>" +  response.issues[i].id + "</p>" + response.issues[i].fields.description + "</div><br>";
  }
  return results;
}
...
checkProjectExists = () => {
  return new Promise(function(resolve, reject) {
    try {
      resolve(make_request("https://jira.secondlife.com/rest/api/2/project/SUN", "json"));
    }
    catch (errorMessage) {
